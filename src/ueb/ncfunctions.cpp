...
int Write_uebaggTS_toNC(
    ...
    // Open netcdf file.
    if ((retncval = nc_open(FileName, NC_WRITE, &ncid)
        )) //|NC_MPIIO, MPI::COMM_WORLD, MPI::DEBUG_NULL, &ncid)))
        ERR(retncval);
    // get variable id
    if ((retncval = nc_inq_varid(ncid, VarName, &v_varid)))
        ERR(retncval);
...
// creates 3D netcdf in PARALLEL mode, and stores dimension variables for UEB aggregated outputs;
// some attributes are copied from the watershed netCDF
#ifdef MPI_PARALLEL
...
#endif // MPI_PARALLEL
{
    ...
    // Create netcdf file.
#ifdef MPI_PARALLEL
    if ((retncval = nc_create_par(
             FileName,
             NC_NETCDF4 | NC_MPIIO | NC_CLOBBER,
             inpComm,
             inpInfo,
             &ncid_out
         )))
#else
    if ((retncval = nc_create(FileName, NC_NETCDF4 | NC_WRITE | NC_CLOBBER, &ncid_out)))
#endif // MPI_PARALLEL
        ERR(retncval);
    //?? set fill on
    ...
    for (int ivar = 0; ivar < naggOut; ivar++) {
        if (retncval = nc_def_var(
                ncid_out,
                (const char*)aggOut[ivar].symbol,
                NC_FLOAT,
                NDIMS,
                dimids,
                &v_varid
        ...
        if (retncval = nc_put_att_text(
                ncid_out,
                v_varid,
                "units",
                strlen((const char*)aggOut[ivar].units),
                (const char*)aggOut[ivar].units
            ))
            ERR(retncval);
...
#endif // MPI_PARALLEL
{
    ...
    if (retncval = nc_copy_att(ncid, pvarid, "grid_mapping", ncid_out, v_varid))
        ERR(retncval);
    /*if (retncval = nc_put_att_text(ncid_out, v_varid, "grid_mapping", , (const
       char*)grid_mappingValue)) ERR(retncval);*/
    if (retncval = nc_inq_varid(ncid, (const char*)grid_mappingValue, &gridmap_id))
        ERR(retncval);
    if (retncval = nc_copy_var(ncid, gridmap_id, ncid_out))
        ERR(retncval);
...
#ifdef MPI_PARALLEL
    if ((retncval =
             nc_create_par(FileName, NC_NETCDF4 | NC_MPIIO | NC_CLOBBER, inpComm, inpInfo, &ncid)))
#else
    if ((retncval = nc_create(FileName, NC_NETCDF4 | NC_WRITE | NC_CLOBBER, &ncid)))
#endif // MPI_PARALLEL
        ERR(retncval);
    // set fill on
...
int read3DNC_Contiguous(
    ...
    nc_type varType;
    // array of dimensions
    int pdimids[3]; // NC_MAX_DIMS]; 3D file only being read here; expected to get error message
                    // otherwise
    // dimension names
    char pdim_Names[80];
...
int read3DNC(
    ...
    nc_type varType;
    // array of dimensions
    int pdimids[3]; // NC_MAX_DIMS]; 3D file only being read here; expected to get error messege
                    // otherwise
    size_t start[3], count[3];
    // Open the file.
#ifdef MPI_PARALLEL
    ...
    for (size_t j = 0; j < Nydim; j++) pvar_inp[j] = new float[Nydim];
#ifdef MPI_PARALLEL
    // parallel access mode
    if ((retncval = nc_var_par_access(ncid, pvarid, NC_COLLECTIVE)))
        ERR(retncval);
    ...
    for (size_t kt = 0; kt < Ntdim; kt++) {
        ...
        for (size_t i = 0; i < Nydim; i++)
            for (size_t j = 0; j < Nxdim; j++) pvar_in[kt][i][j] = pvar_inp[i][j];
        // ncfunc_ss  <<"step no %d\n",k);
    } /* next record */
    // free temporary matrix pvar_inp
    for (size_t i = 0; i < Nydim; i++) delete[] pvar_inp[i];
    delete[] pvar_inp;
    // close netcdf file
    if ((retncval = nc_close(ncid)))
        ERR(retncval);
...
int read2DNC(
    ...
    nc_type varType;
    // array of dimensions
    int pdimids[2]; // NC_MAX_DIMS]; 2D file only being read here; expected to get error message
                    // otherwise
    // size_t start[3], count[3];
    // Open the file.
    if ((retncval = nc_open_par(FILE_NAME, NC_MPIIO, inpComm, inpInfo, &ncid)))
    ...
    for (size_t j = 0; j < Nydim; j++) pvar_in[j] = new float[Nxdim];
    ...
    for (int nj = 0; nj < Nydim; nj++) {
        ...
        if ((retncval = nc_get_vara_float(ncid, pvarid, start, count, &ta_inp[0])))
            ERR(retncval);
        for (int ix = 0; ix < Nxdim; ix++) pvar_in[nj][ix] = ta_inp[ix];
        // ncfunc_ss  <<"step no %d\n",nj);
    } /* next record */
    /*ycorvar = new float[Nydim];
    xcorvar = new float[Nxdim];
...
) //, std::set<int> zValues,  float * z_ycor, float *z_xcor)
{
    ...
    for (size_t j = 0; j < Nydim; j++) pvar_in[j] = new int[Nxdim];
    ycorvar = new float[Nydim];
    xcorvar = new float[Nxdim];
    //
    // parallel access mode
...
{
    // ids for variable, axes,...
    int retncval = 0, ncid = 0, pvarid = 0, pxvarid = 0, pyvarid = 0, ptvarid = 0, pxid = 0,
        pyid = 0, ptid = 0, ndims = 0;
    // variable data type
    // nc_type varType;
    // array of dimensions
    int pdimids[3]; // NC_MAX_DIMS]; 3D file only being read here; expected to get error messege
...
#ifdef MPI_PARALLEL
    if ((retncval =
             nc_create_par(FILE_NAME, NC_NETCDF4 | NC_MPIIO | NC_CLOBBER, inpComm, inpInfo, &ncid)))
#else
    if ((retncval = nc_create(FILE_NAME, NC_NETCDF4 | NC_WRITE | NC_CLOBBER, &ncid)))
#endif // MPI_PARALLEL
        ERR(retncval);
    ...
    // #_6 tbc 5.19.13
    if ((retncval = nc_def_var(ncid, tcor_NAME, NC_DOUBLE, 1, &ptid, &ptvarid)))
        ERR(retncval);
    if ((retncval = nc_def_var(ncid, ycor_NAME, NC_DOUBLE, 1, &pyid, &pyvarid)))
        ERR(retncval);
    if ((retncval = nc_def_var(ncid, xcor_NAME, NC_DOUBLE, 1, &pxid, &pxvarid)))
        ERR(retncval);
    /* The dimids array is used to pass the dimids of the dimensions of
    the netCDF variables.  In C, the unlimited dimension must come first on the list of dimids.
    ...
    *//*	 
    ...
    for (size_t j = 0; j < Nydim; j++) pvar_outp[j] = new float[Nydim];
    ...
    for (size_t kt = 0; kt < Ntdim; kt++) {
        for (size_t i = 0; i < Nydim; i++)
            for (size_t j = 0; j < Nxdim; j++) pvar_outp[i][j] = pvar_out[kt][i][j];
        start[0] = kt;
    ...
    for (size_t i = 0; i < Nydim; i++) delete[] pvar_outp[i];
    delete[] pvar_outp;
    // close netcdf file
    if ((retncval = nc_close(ncid)))
        ERR(retncval);
