...
#else
int WriteTSto3DNC_Block(const char* FileName, const char* VarName, int dimOrd, int *YindArr, int *XindArr, int bSize, int Nt_dim, float** var_inp)  //ydim, xdim =the coordinate point the data to be written; Nt_dim =the length of the TS
#endif //MPI_PARALLEL
{
	// IDs for the netCDF file, dimensions, and variables. 
	int ncid = 0, y_dimid = 0, x_dimid = 0, t_dimid = 0;
...
int read3DNC(const char* FILE_NAME, const char* VAR_NAME, const char* xcor_NAME, const char* ycor_NAME, const char* tcor_NAME, 
                    float* xcorvar, float* ycorvar, float* tcorvar, float*** pvar_in
#ifdef MPI_PARALLEL
		    , MPI::Intracomm inpComm, MPI::Info inpInfo
#endif //MPI_PARALLEL
		    )
