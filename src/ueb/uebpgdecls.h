...
struct params {
    float irad, ireadalb, tr, ts, ems, cg, z, zo, rho, rhog, lc, ks, de, avo, anir0, lans, lang,
        wlf, rd1, dnews, emc, alpha, alphal, gpar, uc, as, Bs, lambda, rimax, wcoeff, apar, cpar;
};
...
#define ERR(e)                                                          \
    {                                                                   \
        ...
        (Logger::GetInstance())->Log(ncfunc_ss.str(), LogLevel::ERROR); \
        ncfunc_ss.str("");                                              \
        return 2;                                                       \
    }

// creates 3D netcdf and stores dimension variables for a given UEB output; called once for a given
// output netcdf attributes are copied from the 2D (watershed) netCDF which the 3D netCDF spatial
// dimension follow
#ifdef MPI_PARALLEL
int create3DNC_uebOutputs(
    const char *FileName,
    const char *VarName,
    const char *varUnits,
    const char *tName,
    const char *tUnits,
    const char *tlong_name,
    const char *tcalendar,
    int Nt_dim,
    int dimOrd,
    float *t_inp,
    float *fillVal,
    const char *ws_FileName,
    const char *ws_VarName,
    const char *yName,
    const char *xName,
    MPI::Intracomm inpComm,
    MPI::Info inpInfo
);
#else
int create3DNC_uebOutputs(
...
#endif // MPI_PARALLEL
// creates 3D netcdf in parallel and stores dimension variables for UEB aggregated outputs; some
// attributes are copied from the watershed netCDF
#ifdef MPI_PARALLEL
int create3DNC_uebAggregatedOutputs(
    const char *FileName,
    aggOutput *aggOut,
    int naggOut,
    const char *tName,
...
int WriteTSto3DNC(
    ...
    MPI::Intracomm inpComm,
    MPI::Info inpInfo
); // ydim, xdim =the coordinate point the data to be written; Nt_dim =the length of the TS
#else
int WriteTSto3DNC(
    const char *FileName,
    const char *VarName,
...
int Write3DNC(
    ...
    MPI::Intracomm inpComm,
    MPI::Info inpInfo
); // int &t_dimid, int &y_dimid, int &x_dimid) //time, y, x
#else
int Write3DNC(
    const char *FileName,
    const char *VarName,
...
int read3DNC(
    ...
    const char *VAR_NAME,
    const char *xcor_NAME,
    const char *ycor_NAME,
    const char *tcor_NAME,
    float *xcorvar,
    float *ycorvar,
    float *tcorvar,
    float ***pvar_in,
...
int readwsncFile(
    ...
    MPI::Intracomm inpComm,
    MPI::Info inpInfo
); // , std::set<int> zValues, float * z_ycor, float *z_xcor);
#else
int readwsncFile(
    const char *FILE_NAME,
    const char *VAR_NAME,
...
#endif // MPI_PARALLEL
// function to read single column/rod along time dimension from 3D netcdf file, for given y , x
// coordinates
#ifdef MPI_PARALLEL
int readNC_TS(
    const char *FILE_NAME,
    ...
    MPI::Intracomm inpComm,
    MPI::Info inpInfo
); //, int xstride); //double* tcorvar, double*** pvar_in)
#else
int readNC_TS(
    const char *FILE_NAME,
    const char *VAR_NAME,
...
#endif // MPI_PARALLEL
// function to read multiple blocks of single column/rod along time dimension from 3D netcdf file,
// for given y , x coordinate arrays
#ifdef MPI_PARALLEL
int readNC_TS_Block(
    const char *FILE_NAME,
    const char *VAR_NAME,
    ...
    int *XindArr,
    int bSize
); // /*float* &tcorvar, int ydim, int xdim, */ int tIndx)
#else
int readNC_TS_Block(
    const char *FILE_NAME,
    const char *VAR_NAME,
...
float ***create3DArrayblock_Contiguous(
    int nt,
    int nr,
    int nc
); // inputs: no. of rows and no. of colos , height/time (dimensions) of matrix
...
void readOutputControl(
    const char *outputconFile,
    pointOutput *&pOut,
    ncOutput *&ncOut,
    aggOutput *&aggOut,
    int &npout,
...
void RUNUEB(
    float *RegArray[13],
    float statesiteValues[],
    float paramValues[],
    float **&OutVarValues,
    int modelStartDate[],
    double modelStartHour,
    int modelEndDate[],
    double modelEndHour,
    double modelDT,
    double UTCOffset
...
// COMPUTES CALENDAR DATE AND time, GIVEN JULIAN DATE.  INPUT JULIAN DATE CAN BE BASED ON ANY
// UT-LIKE time SCALE
...
//      Routine to correct energy and mass fluxes when numerical overshoots dictate that W was
//      changed in the calling routine - either because W went negative or due to the liquid
//      fraction being held constant.
void PREHELP(
    float W1,
    float W,
    float DT,
...
//     Calculates the vapour pressure at a specified temperature over water or ice depending upon
//     temperature.  Temperature is celsius here.
float svp(float T);
//     Calculates the vapour pressure at a specified temperature over water using polynomial from
//     Lowe (1977).
float svpw(float T);
//     Calculates the vapour pressure at a specified temperature over ice using polynomial from Lowe
//     (1977).
float svpi(float T);
